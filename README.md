...
| /init            | 初始化爬虫，若有登录`token`会首先收集登录信息并存储在数据库中               | `token`：`/login`返回url中的`token`参数（无则不执行收集登录信息相关逻辑）                             | 无额外参数                                                                                                                                                                                                                                   | 需在login之后调用                                                            | 需要                           | 不再自动获取关注博主信息，改由定时任务触发 |
...
| /feedback        | 反馈视频不好，全部插件可以据此改进                                          | 无额外参数                                                                                            | `platform`：工作平台<br/>`video`：后端发出的视频数据，原样送回即可<br/>`score`：客户打分，正代表喜欢，不超过±16<br/>等，详细见`api/webAPIs/frontend/PlatformFormat::FeedBack`结构体<br/>`category`：视频所属类别<br/>`quality`：视频质量打分 | 不会有回信，参数中列举部分为必须                                             | 否                             |                                            |
