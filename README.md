...
#### 当前工作流
3.0版本开始，项目将“前端请求”和“真实平台爬取”拆开处理。前端访问根路径并携带`category`时，后端不会直接向**BrowserManager**发起搜索，而是先从PostgreSQL中的预爬取候选池读取已经准备好的视频，再交给插件进行最终筛选和返回。这样用户刷新、插件筛选较严格等行为不会直接放大成对B站的实时搜索请求。

候选池由后台定时线程负责补充。程序启动时会加载插件通过`scheduleCrawl()`注册的定时爬取任务，轮询当前拥有平台Handler的客户端，并按`schedule_crawl_internal`配置的平均间隔执行；实际间隔会乘以0.8到1.2的随机抖动，避免所有任务固定时间集中触发。定时任务会调用`crawlAndStore()`，通过各平台Handler生成`BrowseWorker`并交给**BrowserManager**执行，直到`Client::crawlEnough()`判断对应任务的候选视频数量已经足够。

预爬取结果会写入`client_precrawl_videos`表（开发模式下为`develop_client_precrawl_videos`），按客户端、平台、任务关键字、任务模式、发布时间范围和视频key去重。候选视频默认保留3天；用户请求返回视频后会增加`recommend_count`，后续会优先使用未推荐过的视频，单个视频推荐次数达到5次后会被清理。因此，生产环境中的核心数据流是：后台低频补充候选池，前端请求只消费本地候选池，`prepared`参数从3.0版本开始不再作为主要预加载机制使用。
...
| /init-my-account | 使用程序对账号进行一系列操作，以期望调整你的账号中B站某些视频直接的相关比例 | 无额外参数                                                         | `platform`：工作平台（空表示全部平台，工作情况下不得为空）<br/>`name`：工作模板名称，有这个参数就工作，无则是获取全部可用模板                                                              | 除了获取模式，工作模式不会有回信                                      | 否                 |    |
| /feedback        | 反馈视频不好，全部插件可以据此改进                      | 无额外参数                                                         | `platform`：工作平台<br/>`video`：后端发出的视频数据，原样送回即可<br/>`score`：客户打分，正代表喜欢，不超过±16<br/>等，详细见`api/webAPIs/frontend/PlatformFormat::FeedBack`结构体 | 不会有回信，参数中列举部分为必须                                      | 否                 |    |
...
| prepared | 是否是程序自动触发的预先加载 | 此时不会有回信，直接内部内存存储数据用于后续请求加速返回；且不需要传入任何值，只识别有无这个标签<br/>3.0版本开始已废弃 |
