...
function toggleCategory(category: Category) {
  ...
  if (rotate[category.name] && category.videos.length <= 0) refreshVideos(category)
}
...
<template>
  ...
  <p
    v-else-if="props.categories.length <= 0 && !props.loadFinished"
    class="text-5xl font-bold text-accent"
  >
    正在加载视频数据，请稍等...
  </p>
  <p
