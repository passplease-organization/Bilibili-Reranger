...
| /init         | Initialize the crawler; if a login `token` exists, login information is collected and stored in the database first | `token`: the `token` parameter returned in the URL by `/login` (if absent, login-info collection is skipped)                                                         | No extra parameters                                                                                                                                             | Must be called after login                                                                                    | Required                                 | Won't triggered to gather subscribers by hand, instead, triggered by timer thread |
